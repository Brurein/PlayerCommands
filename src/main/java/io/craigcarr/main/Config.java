...
public class Config{
    ...
    private static final File configFile = new File("config/" + Commands.MOD_ID + "_config.json");
    ...
    public static void loadConfig(){
        try{
            ...
            if(configFile.createNewFile()){
                ...
                fw.close();
                Commands.log(Level.INFO, "Default config generated.");
            }else{
                FileReader fr = new FileReader(configFile);
                INSTANCE = gson.fromJson(fr, Config.class);
                fr.close();
                INSTANCE.generateTransients();
                Commands.log(Level.INFO, "Config loaded.");
                return;
            }
        }catch(Exception e){
        	Commands.log(Level.WARN, "Error loading config, using default values.");
        }
        INSTANCE.generateTransients();
    }
    ...
    public static void saveConfigs(){
        try{
            ...
            fw.append(gson.toJson(INSTANCE));
            fw.close();
            Commands.log(Level.INFO, "Config saved.");
        }catch(Exception e){
        	Commands.log(Level.ERROR, "Error saving config");
        }
    }
